...
import {
  continueSurpriseJourney,
  readSurpriseJourney,
} from "../utils/surpriseJourney";
...
export function CombinationDetailPage() {
  ...
  const surpriseJourney = readSurpriseJourney(location.state, "combination");
  const canShuffleAgain = (surpriseJourney?.poolIds.length ?? 0) > 1;
  const handleShuffleAgain = () => {
    if (!surpriseJourney) return;
    const next = continueSurpriseJourney(surpriseJourney, combination.id);
    if (!next) return;
  ...
  // Related recommendations outside Shop must be fully makeable from the
  // user's inventory. Shop is the only surface that recommends unowned glazes.
  ...
  return (
    <>
      ...
      <PageLayout maxWidth="7xl" padY="6">
        ...
        <div className="mb-3 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          ...
          {/* Action buttons */}
          <div className="flex shrink-0 justify-end gap-2 whitespace-nowrap [&>*]:shrink-0">
            {canShuffleAgain && (
              <button
                type="button"
                onClick={handleShuffleAgain}
                className="p-2 landscape:px-4 landscape:py-2 sm:px-4 sm:py-2 rounded-lg font-medium transition-colors flex items-center gap-2 border bg-terracotta-100 text-terracotta-700 border-terracotta-300 hover:bg-terracotta-200 dark:bg-terracotta-900/30 dark:text-terracotta-300 dark:border-terracotta-700 dark:hover:bg-terracotta-900/50"
                title="Show another random combination using owned glazes"
              >
                <Sparkles size="lg" />
