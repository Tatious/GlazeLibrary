...
export function GlazeDetailPage() {
  ...
  const surpriseJourney = readSurpriseJourney(location.state, "glaze");
  const canShuffleAgain = (surpriseJourney?.poolIds.length ?? 0) > 1;
  const handleShuffleAgain = () => {
    if (!surpriseJourney || !id) return;
    const next = continueSurpriseJourney(surpriseJourney, id);
    if (!next) return;
    navigate(`/glaze/${next.id}`, {
      replace: true,
        ...
        <div className="flex shrink-0 justify-end gap-2 whitespace-nowrap [&>*]:shrink-0">
          {canShuffleAgain && (
            <button
              ...
              title="Show another random owned glaze"
            >
              <Sparkles size="lg" />
