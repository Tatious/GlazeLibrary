...
import { Check, Heart, Shop, Sparkles } from "../components/Icons";
...
export function GlazesPage() {
  ...
  // Inspiration stays grounded in the studio inventory. Search and facets
  // narrow the pool, but switching the visible grid to All/Unowned must not
  // turn the random suggestion into an unowned-glaze recommendation — Shop is
  // the dedicated surface for those.
  const randomizableGlazes = useMemo(
    () => filteredGlazes.filter((g) => myGlazes.glazes[g.id]?.owned),
    [filteredGlazes, myGlazes.glazes],
  );

  const handleRandomGlaze = () => {
    const next = startSurpriseJourney(
      "glaze",
      randomizableGlazes.map((glaze) => glaze.id),
    );
    if (!next) return;
    navigate(`/glaze/${next.id}`, { state: { surprise: next.journey } });
  };
      ...
      <PageLayout maxWidth="7xl" padY="6">
        {!batch.active && filteredGlazes.length > 0 && (
          <div className="flex items-center justify-end gap-2 mb-3">
            {/* A lightweight inspiration path that respects the current search
                and facets while only recommending something in inventory. */}
            {randomizableGlazes.length > 0 && (
              <button
                type="button"
                onClick={handleRandomGlaze}
                ...
                title={`Pick a random owned glaze from these ${randomizableGlazes.length} results`}
              >
