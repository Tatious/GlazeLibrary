...
# CRITICAL: strip frontend-only scripts and @tatious/ui from the PACKAGE's
# package.json. The built SPA already contains the shared UI; Azure only installs
# server runtime dependencies. Reconcile the copied, tested lock after trimming.
...
node -e 'const fs=require("fs"),f="deploy_pkg/package.json",p=JSON.parse(fs.readFileSync(f));for(const k of ["build","build:prod","copy-static"])delete p.scripts[k];delete p.dependencies["@tatious/ui"];fs.writeFileSync(f,JSON.stringify(p,null,2)+"\n")'
node -e 'const fs=require("fs"),f="deploy_pkg/package-lock.json",p=JSON.parse(fs.readFileSync(f));delete p.packages?.["../common/TatiousUI"];delete p.packages?.["node_modules/@tatious/ui"];fs.writeFileSync(f,JSON.stringify(p,null,2)+"\n")'
(cd deploy_pkg && npm install --package-lock-only --omit=dev --ignore-scripts)
