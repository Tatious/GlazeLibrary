...
#   3. Strip build scripts and the frontend-only shared UI dependency from the
#      package's package.json so Oryx does not try
...
node -e 'const fs=require("fs"),f=process.argv[1],p=JSON.parse(fs.readFileSync(f));for(const k of ["build","build:prod","copy-static"])delete (p.scripts||{})[k];delete p.dependencies["@tatious/ui"];fs.writeFileSync(f,JSON.stringify(p,null,2)+"\n")' "$PKG_DIR/package.json"
node -e 'const fs=require("fs"),f=process.argv[1],p=JSON.parse(fs.readFileSync(f));delete p.packages?.["../common/TatiousUI"];delete p.packages?.["node_modules/@tatious/ui"];fs.writeFileSync(f,JSON.stringify(p,null,2)+"\n")' "$PKG_DIR/package-lock.json"
( cd "$PKG_DIR" && npm install --package-lock-only --omit=dev --ignore-scripts )
